...
function handleContentImageLoad(img) {
    ...
    img.style.opacity = '1';
    const wrap = img.parentElement; // .img-wrap
    const shimmer = wrap.querySelector('.image-shimmer');
...
function handleContentImageError(img) {
    ...
    const container = img.closest('.blog-content-image') || img.parentElement;
...
class MarkdownParser {
    ...
    markdownToHtml(markdown) {
        ...
        html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, src) => {
            ...
            const captionHtml = alt ? `<div class="img-caption">${alt}</div>` : '';
            return `<div class="blog-content-image"><div class="img-wrap"><div class="image-shimmer"></div><img src="${imageSrc}" alt="${alt}" style="opacity: 0; transition: opacity 0.3s ease;" onload="handleContentImageLoad(this)" onerror="handleContentImageError(this)">${captionHtml}</div></div>`;
        });
        ...
        // Paragraphs - identify and mark text that precedes headers
