...
We ran a smooth test with **800 items** (with rigidbodies), **250 NPCs** (aggro'd), and **25 player connections**. Next, with 250 players. It looks suitably chaotic, we'll share some footage soon.
...
Fractium will have 3 islands you can travel between. This is what that looks like at the moment in our UI (we want fog of war on the map eventually, though):
![Islands](devblog2-images/islands.png)

Unlike Minecraft/Rust, and unlike 2 weeks ago, islands now only generate **from the seed** on the server/host. The clients recieve the raw terrain and rock placements rather than trying to generate the same exact way as the server. This makes consistency easy as developers, but upsetting for players who want to make cinematic replays, who might expect the seed to look the same way every time.

Rylan also noticed that the terrain may not be capturing the detail generated by the heightmap. For example, our 3D terrains don't seem to have the sand dunes observable in this texture. This is something we are tracking now for a future investigation.
...
Soon, this will host a wiki and convenient documentation for server owners. All pages on this website are generated from markdown and GitHub source pushes are reflected automatically; it's pretty efficient.
